...
export function parseCnbTextResponse(text: string): CnbParsedData {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (rate <= 0) {
      throw new Error(`Invalid exchange rate (must be positive) on line ${i + 1}: ${line}`)
    }
...
export function findCurrencyRate(rates: CurrencyRate[], currencyCode: string): CurrencyRate | null {
