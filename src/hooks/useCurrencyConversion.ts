...
import { calculateCurrencyConversion, validateConversionInput } from '@/utils/conversionCalculator'
...
export function useCurrencyConversion(
  ...
  const validateAndCalculate = useCallback(() => {
    ...
    const validation = validateConversionInput(sanitizedAmount, sanitizedCurrency, exchangeRates)
    ...
    try {
      ...
      setConversionResult(null)
      setErrors({
        conversion: error instanceof Error ? error.message : 'Conversion failed'
      })
    } finally {
  ...
  }, [amount, selectedCurrency, exchangeRates, autoCalculate, debouncedCalculate])
